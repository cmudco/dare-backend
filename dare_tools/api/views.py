...
from dare_tools.api.serializers import (DareToolExecutionSerializer,
                                        DareToolSerializer)
...
class DareToolViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        # is_listed, not is_active: a tool another control already owns still
        # runs and still records its executions, it just is not offered here
        # as a second switch for the same thing.
        return DareTool.active_objects.filter(is_active=True, is_listed=True)
