...
from dare_tools.constants import ExecutionStatus, ToolCategory
...
class DareTool(BaseModel):
    ...
    name = models.CharField(
        max_length=100, help_text="Display name for this tool (e.g., 'Create Diagram')"
    )
    slug = models.SlugField(
        unique=True, help_text="URL-friendly identifier (e.g., 'create_diagram')"
    )
    ...
    is_listed = models.BooleanField(
        default=True,
        help_text=(
            "Whether this tool is offered in the tool drawer. False for tools "
            "another control already owns — search_sessions belongs to the "
            "conversation's Memory toggle, so a second checkbox for it would "
            "do nothing on its own. Distinct from is_active, which means the "
            "tool can run at all: an unlisted tool still executes and still "
            "records its audit row."
        ),
    )
...
class DareToolExecution(BaseModel):
    ...
    tool_call_id = models.CharField(
        max_length=255, blank=True, help_text="Unique ID from LLM tool call"
    )
