...
def _render_config(project):
    ...
    return (
        ...
        "\n"
        "# Pinned. A profile without its own model inherits the runtime default,\n"
        "# so `hermes model` on the box would silently re-point every project.\n"
        "model:\n"
        f"  default: {settings.HERMES_PROFILE_MODEL}\n"
        f"  provider: {settings.HERMES_PROFILE_MODEL_PROVIDER}\n"
        "\n"
        "platform_toolsets:\n"
        "  api_server:\n"
        f"{toolsets}\n"
