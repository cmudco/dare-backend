...
class LLM(models.Model):
    name = models.CharField(
        max_length=255, help_text="Display name of the Language Model."
    )
    ...
    is_active = models.BooleanField(
        default=True, help_text="Whether this model is available for new selections."
    )
...
class ModelGroup(models.Model):
    ...
    name = models.CharField(
        max_length=255,
        help_text="Name of the model group (e.g., 'Basic', 'Premium', 'Enterprise').",
    )
    description = models.TextField(
        blank=True,
        null=True,
        help_text="Description of the model group and its capabilities.",
    )
...
class PublicFeedbackSourceCluster(TimeStampMixin):
    ...
    canonical_title = models.CharField(
        max_length=500, help_text="Primary title for this source cluster"
    )
...
class PublicFeedbackSource(TimeStampMixin):
    ...
    cluster = models.ForeignKey(
        PublicFeedbackSourceCluster, on_delete=models.CASCADE, related_name="sources"
    )
    ...
    snippet = models.TextField(blank=True, help_text="Text snippet from search results")

    originating_query = models.CharField(
        max_length=200, blank=True, help_text="The search query that found this source"
    )
...
class Conversation(BaseModel):
    ...
    conversation_id = models.CharField(
        max_length=50, unique=True, help_text="Unique conversation ID."
    )
    title = models.CharField(
        max_length=255, blank=True, null=True, help_text="Title of the conversation."
    )
    ...
    image_generation_enabled = models.BooleanField(
        default=False, help_text="Enable AI image generation for this conversation."
    )
    audio_transcription_enabled = models.BooleanField(
        default=False, help_text="Enable audio transcription for this conversation."
    )
    ...
    selected_library_ids = models.JSONField(
        default=list,
        blank=True,
    ...
    def clone(
        ...
        with transaction.atomic():
            ...
            cloned_conversation = Conversation(
                ...
                selected_file_ids=(
                    self.selected_file_ids.copy() if self.selected_file_ids else []
                ),
                selected_embedding_ids=(
                    self.selected_embedding_ids.copy()
                    if self.selected_embedding_ids
                    else []
                ),
                selected_library_ids=(
                    self.selected_library_ids.copy()
                    if self.selected_library_ids
                    else []
                ),
            ...
            if self.selected_mcp_servers.exists():
                cloned_conversation.selected_mcp_servers.set(
                    self.selected_mcp_servers.all()
                )

            # Clone DARE tool selections
            if self.selected_dare_tools.exists():
                cloned_conversation.selected_dare_tools.set(
                    self.selected_dare_tools.all()
                )
                ...
                for original_message in original_messages:
                    cloned_message = Message(
                        ...
                        sender=(
                            user.email
                            if user
                            and original_message.sender_type == SenderType.PLAYER
...
class Message(BaseModel):
    ...
    sender_type = models.IntegerField(
        choices=SenderType.choices, help_text="Type of sender (User or AI)."
    )
    ...
    output_tokens = models.PositiveIntegerField(
        null=True, blank=True, help_text="Number of output tokens generated by the LLM."
    )
    ...
    feedback_text = models.TextField(
        blank=True, null=True, help_text="Optional feedback text provided by the user."
    )
    ...
    class Meta:
        indexes = [
            ...
            models.Index(
                fields=["conversation", "sender_type"], name="msg_conv_sender_idx"
            ),
        ]
...
class MessageToolCall(BaseModel):
    ...
    tool_name = models.CharField(
        max_length=200, help_text="Name of the tool (e.g., 'channels_list')."
    )
    ...
    result = models.TextField(
        null=True, blank=True, help_text="Result text from tool execution."
    )
...
class WebSearchSource(BaseModel):
    ...
    url = models.URLField(max_length=2048, help_text="The URL of the source.")
    title = models.CharField(
        max_length=500, blank=True, help_text="The title of the source page."
    )
    cited_text = models.TextField(
        blank=True, help_text="The text that was cited from this source (Claude only)."
    )
...
class LearningProgressAssessment(BaseModel):
    ...
    learning_goals = models.TextField(
        blank=True, help_text="Learning goals that were used for this assessment."
    )
...
class Artifact(BaseModel):
    ...
    title = models.CharField(max_length=500, help_text="Title of the artifact.")
    ...
    outline = models.TextField(
        blank=True, default="", help_text="Structured outline of the artifact sections."
    )
    content = models.TextField(
        blank=True, default="", help_text="Generated content of the artifact."
    )

    estimated_sections = models.PositiveIntegerField(
        default=10, help_text="Estimated number of sections in the artifact."
    )
    current_section = models.PositiveIntegerField(
        default=0, help_text="Current section being generated (0 = not started)."
    )
    ...
    version = models.PositiveIntegerField(
        default=1, help_text="Version number, incremented on each modification."
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(
                fields=["conversation", "status"], name="artifact_conv_status_idx"
            ),
        ]
...
class ArtifactCheckpoint(BaseModel):
    ...
    iteration_count = models.PositiveIntegerField(
        default=0, help_text="Number of generation iterations completed."
    )
    state_data = models.JSONField(
        default=dict, help_text="Serialized state data for resuming generation."
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(
                fields=["artifact", "created_at"], name="checkpoint_artifact_idx"
            ),
        ]
...
class Feedback(BaseModel):
    ...
    message = models.TextField(
        blank=True, help_text="Detailed feedback message from the user."
    )
    screenshot = models.TextField(
        blank=True, null=True, help_text="Base64 encoded screenshot (optional)."
    )
    page = models.CharField(
        max_length=500, help_text="Page URL where feedback was submitted."
    )
