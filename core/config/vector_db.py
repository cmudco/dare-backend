...
def get_library_namespace(slug: str) -> str:
    """Generate the dedicated, un-scoped namespace for a shared library corpus.

    Distinct from user namespaces (``user_<id>``) so library vectors are
    isolated and queried without a user filter.
    """
    return f"library{NAMESPACE_SEPARATOR}{slug}"
...
def parse_vector_id(vector_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if len(parts) >= 4 and parts[0] == "file" and parts[2] == "chunk":
            return {"file_id": parts[1], "chunk_index": int(parts[3])}
