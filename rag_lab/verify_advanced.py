...
from core.services.rag import QueryAnalyzer
...
for query in [
    ...
    print(f"QUERY: {query}")
    print(
        f"  intent = {intent}  ->  conditional MMR {'ON' if intent=='exploratory' else 'OFF'}"
    )
    print(f"  keywords = {list(plan.keywords) if plan else '-'}")
    parts = _run_library_search(
        dp, query, [1], max_context_snippets=6, message_obj=None
    )
